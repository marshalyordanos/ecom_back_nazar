...
export async function getGlobalDashboardSummary() {
  ...
  const [
    ...
    prisma.order.count({ where: { createdAt: { gte: currentWindowStart } } }),
    prisma.order.count({ where: { createdAt: { gte: prevWindowStart, lt: currentWindowStart } } }),
    ...
    prisma.payment.count({ where: { createdAt: { gte: currentWindowStart } } }),
    prisma.payment.count({ where: { createdAt: { gte: prevWindowStart, lt: currentWindowStart } } }),
...
export async function getEcommerceHighlights(shopId: string, limit = 3) {
  ...
  for (const item of soldVariants) {
    ...
    const productIds = categoryProductIds.get(rootCategoryId) ?? new Set<string>();
  ...
  const categoryHighlights = Array.from(topLevelById.values())
    .map((category) => {
      ...
      const totalViews = Array.from(categoryProductIds.get(category.id) ?? []).reduce(
        (sum, productId) => sum + (productViewsById.get(productId) ?? 0),
        0,
      );
